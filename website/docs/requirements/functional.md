...
| **REQ-CORE-FUNC-003** | **Launchpad Discovery** | The Mission Control (NovaAdmin) must dynamically query the Gateway to list active sectors. | 1. NovaAdmin calls Gateway `/projects` or `/services` endpoint.<br/>2. Dashboard renders a grid of active sectors.<br/>3. Clicking "NovaAgro" navigates to `agriculture.novaeco.tech`. |
